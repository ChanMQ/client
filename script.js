...
const APP_NAME = "e2e.mom";
const STORAGE_KEYS = {
    session: "e2e_mom_session_v1",
    settings: "e2e_mom_settings_v1",
    ui: "e2e_mom_ui_v1",
};
const SESSION_SECRET_STORAGE_KEY = "e2e_mom_secret_storage_key";
...
function renderPublicDirectory() {
    ...
    state.currentPublicRooms.forEach((room) => {
        ...
        item.innerHTML = `
            <div class="public-avatar">${room.avatar_url && state.settings.showAvatars ? `<img src="${escapeHtml(mxcToSizedHttp(room.avatar_url, 96, 96))}" alt="">` : escapeHtml(initialsFromName(room.name || room.canonical_alias || room.room_id || "P"))}</div>
...
function renderChat() {
    ...
    refs.chatHeaderAvatar.innerHTML = roomAvatarMxc && state.settings.showAvatars
        ? `<img src="${escapeHtml(mxcToSizedHttp(roomAvatarMxc, 80, 80))}" alt="">`
        : escapeHtml(initialsFromName(getRoomName(room)));
...
function renderMessageEvent(event, room, { isMine, shouldShowAuthor }) {
    ...
    row.innerHTML = `
        <div class="member-avatar${isContinuation && !isMine ? " spacer" : ""}">${state.settings.showAvatars && getMemberAvatarMxc(member) ? `<img data-avatar-mxc="${escapeHtml(getMemberAvatarMxc(member))}" alt="">` : escapeHtml(initialsFromName(senderName))}</div>
...
async function hydrateMedia(row, event) {
    ...
    const avatarImg = row.querySelector("img[data-avatar-mxc]");
    if (avatarImg) {
        const avatarMxc = avatarImg.dataset.avatarMxc || "";
        avatarImg.src = await getMediaSrc(avatarMxc);
    }
...
function getMemberAvatarMxc(member) {
    return member?.events?.member?.getContent?.()?.avatar_url
        || member?.getMxcAvatarUrl?.()
        || member?.user?.avatarUrl
        || "";
}

function mxcToSizedHttp(mxcUrl, width = 96, height = 96, method = "crop") {
    if (!state.client || !mxcUrl) return "";
    return state.client.mxcUrlToHttp(mxcUrl, width, height, method, false, true, true)
...
function maybeAutoScroll(force = false) {
    const targetTop = refs.messageContainer.scrollHeight;
    if (force) {
        refs.messageContainer.scrollTo({ top: targetTop, behavior: "auto" });
        return;
    }
    ...
    if (nearBottom) refs.messageContainer.scrollTo({ top: targetTop, behavior: "smooth" });
}
